...
namespace WeifenLuo.WinFormsUI.Docking
{
	internal class VS2005AutoHideStrip : AutoHideStripBase
	{
		...
		private void DrawTab(Graphics g, TabVS2005 tab)
		{
            ...
            { 
                //ワールド変換を平行移動
                g.TranslateTransform(rectText.X + 16,rectText.Y - 1);

                //ワールド変換行列を90度回転する
                g.RotateTransform(90F);               

                //文字列を(0, 0)に描画
                g.DrawString(content.DockHandler.TabText, TextFont, new SolidBrush(textColor), 0, 0); 
            }
            else if (dockState == DockState.DockRightAutoHide)
            {
                //ワールド変換を平行移動
                g.TranslateTransform(rectText.X + 4 , rectText.Y + rectText.Height - 2);
                ...
                //文字列を(0, 0)に描画
                g.DrawString(content.DockHandler.TabText, TextFont, new SolidBrush(textColor), 0, 0); 
                //g.FillRectangle(new SolidBrush(Color.Black), 0, 0, 5, 5);
